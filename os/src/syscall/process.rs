...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let mut_time = translated_refmut(token, ts);
    *mut_time = TimeVal {
        sec: us / 1_000_000,
        usec: us % 1_000_000,
    };
...
pub fn sys_mmap(start: usize, len: usize, prot: usize) -> isize {
    ...
    current_task().unwrap().inner_exclusive_access().memory_set.mmap(start, len, map_perm)
}
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    let path = translated_str(current.get_user_token(), path);
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        ...
        current.inner_exclusive_access().children.push(tcb.clone());
        add_task(tcb.clone());
        new_pid as isize
    } else {
        -1
